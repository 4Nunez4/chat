...
export const createChat = async (req, res) => {
  try {
    ...
    const imagen_chat = req.file ? req.file.filename : "";
    const remitente = fk_id_usuario; // Puedes obtener el nombre de usuario real si lo deseas

    // Enviar el mensaje a través de Socket.IO
    socket.emit("message", { mensaje: mensaje_chat, remitente, idSubasta: fk_id_subasta, imagen: imagen_chat });

    res.status(200).json({ status: 200, message: "Mensaje enviado con éxito." });
  } catch (error) {
